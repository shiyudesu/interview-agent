...
- Operation 的幂等范围等于命令类型，并通过 `(owner_user_id, idempotency_scope, idempotency_key)` 唯一；Operation ID、所有者、面试、范围、键、命令类型、期望版本和输入创建后不可修改，面试仍存在时也禁止直接删除后重建。删除面试时的外键级联不受影响。`processing` 状态必须持有有效时间范围的租约，其他状态不得保留租约时间。
